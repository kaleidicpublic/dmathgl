...
else
{
	...
	void DefaultPlotParam()			{	mgl_set_def_param(gr);	}
	/// Set name of plot for saving filename
	void SetPlotId(const (char*) id)	{	mgl_set_plotid(gr,id);	}
	/// Get name of plot for saving filename
	const (char*) GetPlotId()	{	return mgl_get_plotid(gr);	}

	/// Ask to stop drawing
	void Stop(bool stop=true)	{	mgl_ask_stop(gr, stop);	}
	/// Check if plot termination is asked
	bool NeedStop()	{	return mgl_need_stop(gr);	}
	/// Set callback function for event processing
	void SetEventFunc(void (*func)(void *), void *par=NULL)
	{	mgl_set_event_func(gr, func, par);	}

	/// Set the transparency on/off.
	void Alpha(bool enable)			{	mgl_set_alpha(gr, enable);	}
	/// Set default value of alpha-channel
	void SetAlphaDef(double alpha)	{	mgl_set_alpha_default(gr, alpha);	}
	/// Set the transparency type (0 - usual, 1 - glass, 2 - lamp)
	void SetTranspType(int type)		{	mgl_set_transp_type(gr, type);	}

	/// Set the using of light on/off.
	void Light(bool enable)			{	mgl_set_light(gr, enable);	}
	/// Switch on/off the specified light source.
	void Light(int n,bool enable)	{	mgl_set_light_n(gr, n, enable);	}
	/// Use diffusive light (only for local light sources) -- OBSOLETE
	void SetDifLight(bool dif)		{	mgl_set_light_dif(gr, dif);	}
	/// Add a light source.
	void AddLight(int n, mglPoint p, char col='w', double bright=0.5, double ap=0)
	{	mgl_add_light_ext(gr, n, p.x, p.y, p.z, col, bright, ap);	}
	void AddLight(int n, mglPoint r, mglPoint p, char col='w', double bright=0.5, double ap=0)
	{	mgl_add_light_loc(gr, n, r.x, r.y, r.z, p.x, p.y, p.z, col, bright, ap);	}
	/// Set ambient light brightness
	void SetAmbient(double i)			{	mgl_set_ambbr(gr, i);	}
	/// Set diffusive light brightness
	void SetDiffuse(double i)			{	mgl_set_difbr(gr, i);	}
	/// Set the fog distance or switch it off (if d=0).
	void Fog(double d, double dz=0.25)	{	mgl_set_fog(gr, d, dz);		}

	/// Set relative width of rectangles in Bars, Barh, BoxPlot
	void SetBarWidth(double width)	{	mgl_set_bar_width(gr, width);	}
	/// Set default size of marks (locally you can use "size" option)
	void SetMarkSize(double size)		{	mgl_set_mark_size(gr, size);	}
	/// Set default size of arrows (locally you can use "size" option)
	void SetArrowSize(double size)	{	mgl_set_arrow_size(gr, size);	}
	/// Set number of mesh lines (use 0 to draw all of them)
	void SetMeshNum(int num)			{	mgl_set_meshnum(gr, num);	}
	/// Set number of visible faces (use 0 to draw all of them)
	void SetFaceNum(int num)			{	mgl_set_facenum(gr, num);	}

	/// Set cutting for points outside of bounding box
	void SetCut(bool cut)				{	mgl_set_cut(gr, cut);	}
	...
	void CutOff(const (char*) EqC)		{	mgl_set_cutoff(gr, EqC);	}

	/// Set default font size
	void SetFontSize(double size)		{	mgl_set_font_size(gr, size);	}
	/// Set default font style and color
	void SetFontDef(const (char*) fnt)		{	mgl_set_font_def(gr, fnt);	}
	...
	void SetFontSizeCM(double cm, int dpi=72)	{	SetFontSizePT(cm*28.45f,dpi);	}
	/// Set FontSize by size in inch and picture DPI (default is 0.22 in = 16 pt)
	void SetFontSizeIN(double in, int dpi=72)	{	SetFontSizePT(in*72.27f,dpi);	}
	/// Load font from file
	void LoadFont(const (char*) name, const (char*) path=NULL)
	{	mgl_load_font(gr, name, path);	}
	/// Copy font from another mglGraph instance
	void CopyFont(const mglGraph *GR)	{	mgl_copy_font(gr, GR->gr);}
	/// Restore font (load default font for new HMGL objects)
	void RestoreFont()				{	mgl_restore_font(gr);	}
	/// Set to use or not text rotation
	void SetRotatedText(bool rotated)	{	mgl_set_rotated_text(gr, rotated);	}
	/// Set default font for all new HMGL and mglGraph objects
	static void SetDefFont(const (char*) name, const (char*) path=NULL)	{	mgl_def_font(name,path);	}

	/// Set default palette
	void SetPalette(const (char*) colors)	{	mgl_set_palette(gr, colors);	}
	/// Set default color scheme
	void SetDefScheme(const (char*) sch)	{	mgl_set_def_sch(gr, sch);	}

	/// Sets RGB values for color with given id
	static void SetColor(char id, double r, double g, double b)	{	mgl_set_color(id, r, g, b);	}
	/// Set mask for face coloring as array of type 'ubyte[8]'
	static void SetMask(char id, const (char*) mask)	{	mgl_set_mask(id, mask);	}
	/// Set mask for face coloring as uint64_t number
	static void SetMask(char id, uint64_t mask)	{	mgl_set_mask_val(id, mask);	}
	/// Set default mask rotation angle
	void SetMaskAngle(int angle)	{	mgl_set_mask_angle(gr, angle);	}

	/// Get last warning code
	int  GetWarn()	{	return mgl_get_warn(gr);}
	/// Set warning code ant fill message
	void SetWarn(int code, const (char*) info)	{	mgl_set_warn(gr,code,info);	}
	/// Get text of warning message(s)
	const (char*) Message()	{	return mgl_get_mess(gr);	}
	/// Set global warning message
	static void SetGlobalWarn(const (char*) text)	{	mgl_set_global_warn(text);	}
	/// Get text of global warning message(s)
	static const (char*) GlobalWarn()	{	return mgl_get_global_warn();	}
	/// Suppress printing warnings to stderr
	static void SuppressWarn(bool on)	{	mgl_suppress_warn(on);	}
	/// Check if MathGL version is valid (return false) or not (return true)
	static bool CheckVersion(const (char*) ver)	{	return mgl_check_version(ver);	}

	/// Set axis range scaling -- simplified way to shift/zoom axis range -- need to replot whole image!
	void ZoomAxis(mglPoint p1=mglPoint(0,0,0,0), mglPoint p2=mglPoint(1,1,1,1))
	...
	void SetRanges(const mglDataA &xx, const mglDataA &yy, const mglDataA &zz, const mglDataA &cc)
	...
	void SetAutoRanges(double x1, double x2, double y1=0, double y2=0, double z1=0, double z2=0, double c1=0, double c2=0)
	...
	void SetCoor(int how)		{	mgl_set_coor(gr, how);	}
	/// Set to draw Ternary axis (triangle like axis, grid and so on)
	void Ternary(int val)		{	mgl_set_ternary(gr, val);	}

	/// Set to use or not tick labels rotation
	void SetTickRotate(bool val)	{	mgl_set_tick_rotate(gr,val);	}
	/// Set to use or not tick labels skipping
	void SetTickSkip(bool val)	{	mgl_set_tick_skip(gr,val);	}
	/// Set tick length
	...
	void SetTicksTime(char dir, double d=0, const (char*) t="")
	...
	void SetTicksVal(char dir, const mglDataA &v, const (char*) lbl, bool add=false)
	...
	void SetTicks(char dir, double d=0, int ns=0, double org=mglNaN, const (char*) factor="")
	...
	/// Auto adjust ticks
	void Adjust(const (char*) dir="xyzc")
	...
	void SubPlot(int nx,int ny,int m,const (char*) style="<>_^", double dx=0, double dy=0)
	{	mgl_subplot_d(gr, nx, ny, m, style, dx, dy);	}
	/// Like SubPlot() but "join" several cells
	void MultiPlot(int nx,int ny,int m, int dx, int dy, const (char*) style="<>_^")
	...
	void Push()	{	mgl_mat_push(gr);	}
	/// Pop transformation matrix from stack
	void Pop()	{	mgl_mat_pop(gr);	}

	/// Add title for current subplot/inplot
		void Title(const (char*) title,const (char*) stl="",double size=-2)
	...
	void SetSize(int width, int height)	{	mgl_set_size(gr, width, height);	}
	/// Set plot quality
	void SetQuality(int qual=MGL_DRAW_NORM)	{	mgl_set_quality(gr, qual);	}
	/// Get plot quality
	int GetQuality()	{	return mgl_get_quality(gr);	}
	/// Set drawing region for Quality&4
	void SetDrawReg(long nx=1, long ny=1, long m=0)	{	mgl_set_draw_reg(gr,nx,ny,m);	}
	/// Start group of objects
	void StartGroup(const (char*) name)		{	mgl_start_group(gr, name);	}
	/// End group of objects
	void EndGroup()	{	mgl_end_group(gr);	}
	/// Highlight objects with given id
	void Highlight(int id)	{	mgl_highlight(gr, id);	}

	/// Show current image
	void ShowImage(const (char*) viewer, bool keep=0)
	{	mgl_show_image(gr, viewer, keep);	}
	/// Write the frame in file (depending extension, write current frame if fname is empty)
	void WriteFrame(const (char*) fname=0,const (char*) descr="")
	...
	void WriteBMP(const (char*) fname,const (char*) descr="")
	{	mgl_write_bmp(gr, fname, descr);	}
	/// Write the frame in file using BMP format
	void WriteTGA(const (char*) fname,const (char*) descr="")
	...
	void WriteTEX(const (char*) fname,const (char*) descr="")
...
//	{	mgl_write_x3d(gr, fname, descr);	}
	...
	const ubyte *GetRGB()		{	return mgl_get_rgb(gr);	}
	...
	const ubyte *GetRGBA()	{	return mgl_get_rgba(gr);	}
	...
	const ubyte *GetBackground()	{	return mgl_get_background(gr);	}
	/// Get width of the image
	int GetWidth()	{	return mgl_get_width(gr);	}
	/// Get height of the image
	int GetHeight()	{	return mgl_get_height(gr);}
	...
	void SetObjId(int id)		{	mgl_set_obj_id(gr,id);	}
	/// Get object id
	int GetObjId(long x,long y)	{	return mgl_get_obj_id(gr,x,y);	}
	/// Get subplot id
	int GetSplId(long x,long y)	{	return mgl_get_spl_id(gr,x,y);	}
	/// Check if {\a xs,\a ys} is close to active point with accuracy d, and return its position or -1
	long IsActive(int xs, int ys, int d=1)	{	return mgl_is_active(gr,xs,ys,d);	}

	/// Combine plots from 2 canvases. Result will be saved into this
	void Combine(const mglGraph *g)	{	mgl_combine_gr(gr,g->gr);	}

	/// Clear up the frame
	void Clf(double r, double g, double b)	{	mgl_clf_rgb(gr, r, g, b);	}
	void Clf(const (char*) col)	{	mgl_clf_str(gr, col);	}
	void Clf(char col)	{	mgl_clf_chr(gr, col);	}
	void Clf()	{	mgl_clf(gr);	}
	/// Clear unused points and primitives. Useful only in combination with SetFaceNum().
	void ClearUnused()	{	mgl_clear_unused(gr);	}
	...
	void Rasterize()			{	mgl_rasterize(gr);	}
	...
	void Face(mglPoint p1, mglPoint p2, mglPoint p3, mglPoint p4, const (char*) stl="r")
	...
	void FaceX(mglPoint p, double wy, double wz, const (char*) stl="w", double dx=0, double dy=0)
	...
	void FaceY(mglPoint p, double wx, double wz, const (char*) stl="w", double dx=0, double dy=0)
	...
	void Arc(mglPoint p0, mglPoint pr, mglPoint p1, double a, const (char*) stl="r")
	...
	void Arc(mglPoint p0, mglPoint p1, double a, const (char*) stl="r")
	{	mgl_arc_ext(gr, p0.x,p0.y,p0.z, 0,0,1, p1.x,p1.y,p0.z, a,stl);	}
	/// Draws bitmap (logo) which is stretched along whole axis range
	void Logo(long w, long h, const ubyte *rgba, bool smooth=false, const (char*) opt="")
	{	mgl_logo(gr, w, h, rgba, smooth, opt);	}
	void Logo(const (char*) fname, bool smooth=false, const (char*) opt="")
	...
	void Putsw(double x, double y,const wchar_t *text,const (char*) font=":AC",double size=-1)
	...
	void Putsw(mglPoint p, mglPoint d, const wchar_t *text, const (char*) font=":L", double size=-1)
	{	mgl_putsw_dir(gr, p.x, p.y, p.z, d.x, d.y, d.z, text, font, size);	}
	void Puts(mglPoint p, mglPoint d, const (char*) text, const (char*) font=":L", double size=-1)
	{	mgl_puts_dir(gr, p.x, p.y, p.z, d.x, d.y, d.z, text, font, size);	}

	/// Print text along the curve
	void Text(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) text, const (char*) font="", const (char*) opt="")
	{	mgl_text_xyz(gr, &x, &y, &z, text, font, opt);	}
	void Text(const mglDataA &x, const mglDataA &y, const (char*) text, const (char*) font="", const (char*) opt="")
	{	mgl_text_xy(gr, &x, &y, text, font, opt);	}
	void Text(const mglDataA &y, const (char*) text, const (char*) font="", const (char*) opt="")
	{	mgl_text_y(gr, &y, text, font, opt);	}
	void Text(const mglDataA &x, const mglDataA &y, const mglDataA &z, const wchar_t *text, const (char*) font="", const (char*) opt="")
	{	mgl_textw_xyz(gr, &x, &y, &z, text, font, opt);	}
	void Text(const mglDataA &x, const mglDataA &y, const wchar_t *text, const (char*) font="", const (char*) opt="")
	{	mgl_textw_xy(gr, &x, &y, text, font, opt);	}
	void Text(const mglDataA &y, const wchar_t *text, const (char*) font="", const (char*) opt="")
	...
	void Label(char dir, const (char*) text, double pos=+1, const (char*) opt="")
	{	mgl_label(gr, dir, text, pos, opt);	}
	void Label(char dir, const wchar_t *text, double pos=+1, const (char*) opt="")
	{	mgl_labelw(gr, dir, text, pos, opt);	}

	/// Draw colorbar at edge of axis
	void Colorbar(const (char*) sch="")
	{	mgl_colorbar(gr, sch);	}
	void Colorbar(const (char*) sch,double x,double y,double w=1,double h=1)
	{	mgl_colorbar_ext(gr, sch, x,y,w,h);	}
	/// Draw colorbar with manual colors at edge of axis
	void Colorbar(const mglDataA &val, const (char*) sch="")
	{	mgl_colorbar_val(gr, &val, sch);	}
	void Colorbar(const mglDataA &val, const (char*) sch,double x,double y,double w=1,double h=1)
	...
	void Legend(double x, double y, const (char*) font="#", const (char*) opt="")
	{	mgl_legend_pos(gr, x, y, font, opt);	}
	/// Draw legend of accumulated strings
	void Legend(int where=3, const (char*) font="#", const (char*) opt="")
	{	mgl_legend(gr, where, font, opt);	}
	/// Set number of marks in legend sample
	void SetLegendMarks(int num)		{	mgl_set_legend_marks(gr, num);	}

	/// Draw usual curve {x,y,z}
	void Plot(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) pen="", const (char*) opt="")
	...
	void Plot(const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_plot(gr, &y, pen,opt);	}
	/// Draw tape(s) which rotates as (bi-)normales of curve {x,y,z}
	void Tape(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) pen="", const (char*) opt="")
	{	mgl_tape_xyz(gr, &x, &y, &z, pen, opt);	}
	void Tape(const mglDataA &x, const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_tape_xy(gr, &x, &y, pen,opt);	}
	void Tape(const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_tape(gr, &y, pen,opt);	}
	/// Draw radar chart (plot in curved coordinates)
	void Radar(const mglDataA &a, const (char*) pen="", const (char*) opt="")
	{	mgl_radar(gr, &a, pen, opt);	}
	/// Draw stairs for points in arrays {x,y,z}
	void Step(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) pen="", const (char*) opt="")
	{	mgl_step_xyz(gr, &x, &y, &z, pen, opt);	}
	void Step(const mglDataA &x, const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_step_xy(gr, &x, &y, pen, opt);	}
	void Step(const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_step(gr, &y, pen, opt);	}
	/// Draw curve {x,y,z} which is colored by c (like tension plot)
	void Tens(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &c, const (char*) pen="", const (char*) opt="")
	{	mgl_tens_xyz(gr, &x, &y, &z, &c, pen, opt);	}
	void Tens(const mglDataA &x, const mglDataA &y, const mglDataA &c, const (char*) pen="", const (char*) opt="")
	{	mgl_tens_xy(gr, &x, &y, &c, pen, opt);	}
	void Tens(const mglDataA &y, const mglDataA &c, const (char*) pen="", const (char*) opt="")
	{	mgl_tens(gr, &y, &c, pen, opt);	}
	/// Fill area between curve {x,y,z} and axis plane
	void Area(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) pen="", const (char*) opt="")
	{	mgl_area_xyz(gr, &x, &y, &z, pen, opt);	}
	void Area(const mglDataA &x, const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_area_xy(gr, &x, &y, pen, opt);	}
	void Area(const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_area(gr, &y, pen, opt);	}
	/// Fill area between curves y1 and y2 specified parametrically
	void Region(const mglDataA &y1, const mglDataA &y2, const (char*) pen="", const (char*) opt="")
	{	mgl_region(gr, &y1, &y2, pen, opt);	}
	void Region(const mglDataA &x, const mglDataA &y1, const mglDataA &y2, const (char*) pen="", const (char*) opt="")
	{	mgl_region_xy(gr, &x, &y1, &y2, pen, opt);	}
	/// Fill area (draw ribbon) between curves {x1,y1,z1} and {x2,y2,z2}
	void Region(const mglDataA &x1, const mglDataA &y1, const mglDataA &z1, const mglDataA &x2, const mglDataA &y2, const mglDataA &z2, const (char*) pen="", const (char*) opt="")
	{	mgl_region_3d(gr, &x1, &y1, &z1, &x2, &y2, &z2, pen, opt);	}
	void Region(const mglDataA &x1, const mglDataA &y1, const mglDataA &x2, const mglDataA &y2, const (char*) pen="", const (char*) opt="")
	{	mgl_region_3d(gr, &x1, &y1, NULL, &x2, &y2, NULL, pen, opt);	}
	/// Draw vertical lines from points {x,y,z} to axis plane
	void Stem(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) pen="", const (char*) opt="")
	{	mgl_stem_xyz(gr, &x, &y, &z, pen, opt);	}
	void Stem(const mglDataA &x, const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_stem_xy(gr, &x, &y, pen, opt);	}
	void Stem(const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_stem(gr, &y, pen, opt);	}

	/// Draw vertical bars from points {x,y,z} to axis plane
	void Bars(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) pen="", const (char*) opt="")
	{	mgl_bars_xyz(gr, &x, &y, &z, pen, opt);	}
	void Bars(const mglDataA &x, const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_bars_xy(gr, &x, &y, pen, opt);	}
	void Bars(const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_bars(gr, &y, pen, opt);	}
	/// Draw horizontal bars from points {x,y} to axis plane
	void Barh(const mglDataA &y, const mglDataA &v, const (char*) pen="", const (char*) opt="")
	{	mgl_barh_yx(gr, &y, &v, pen, opt);	}
	void Barh(const mglDataA &v, const (char*) pen="", const (char*) opt="")
	{	mgl_barh(gr, &v, pen, opt);	}
	/// Draw chart for data a
	void Chart(const mglDataA &a, const (char*) colors="", const (char*) opt="")
	{	mgl_chart(gr, &a, colors,opt);	}

	/// Draw Open-High-Low-Close (OHLC) diagram
	void OHLC(const mglDataA &x, const mglDataA &open, const mglDataA &high, const mglDataA &low, const mglDataA &close, const (char*) pen="", const (char*) opt="")
	{	mgl_ohlc_x(gr, &x, &open,&high,&low,&close,pen,opt);	}
	void OHLC(const mglDataA &open, const mglDataA &high, const mglDataA &low, const mglDataA &close, const (char*) pen="", const (char*) opt="")
	{	mgl_ohlc(gr, &open,&high,&low,&close,pen,opt);	}

	/// Draw box-plot (special 5-value plot used in statistic)
	void BoxPlot(const mglDataA &x, const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_boxplot_xy(gr, &x, &y, pen,opt);	}
	void BoxPlot(const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_boxplot(gr, &y, pen,opt);	}
	/// Draw candle plot
	void Candle(const mglDataA &x, const mglDataA &v1, const mglDataA &v2, const mglDataA &y1, const mglDataA &y2, const (char*) pen="", const (char*) opt="")
	{	mgl_candle_xyv(gr, &x, &v1, &v2, &y1, &y2, pen, opt);	}
	void Candle(const mglDataA &v1, const mglDataA &v2, const mglDataA &y1, const mglDataA &y2, const (char*) pen="", const (char*) opt="")
	{	mgl_candle_yv(gr, &v1, &v2, &y1, &y2, pen, opt);	}
	void Candle(const mglDataA &v1, const mglDataA &v2, const (char*) pen="", const (char*) opt="")
	{	mgl_candle_yv(gr, &v1, &v2, NULL, NULL, pen, opt);	}
	void Candle(const mglDataA &y, const mglDataA &y1, const mglDataA &y2, const (char*) pen="", const (char*) opt="")
	{	mgl_candle(gr, &y, &y1, &y2, pen, opt);	}
	void Candle(const mglDataA &y, const (char*) pen="", const (char*) opt="")
	{	mgl_candle(gr, &y, NULL, NULL, pen, opt);	}
	/// Draw cones from points {x,y,z} to axis plane
	void Cones(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) pen="@", const (char*) opt="")
	{	mgl_cones_xyz(gr, &x, &y, &z, pen, opt);	}
	void Cones(const mglDataA &x, const mglDataA &z, const (char*) pen="@", const (char*) opt="")
	{	mgl_cones_xz(gr, &x, &z, pen, opt);	}
	void Cones(const mglDataA &z, const (char*) pen="@", const (char*) opt="")
	{	mgl_cones(gr, &z, pen, opt);	}

	/// Draw error boxes {ex,ey} at points {x,y}
	void Error(const mglDataA &y, const mglDataA &ey, const (char*) pen="", const (char*) opt="")
	{	mgl_error(gr, &y, &ey, pen, opt);	}
	void Error(const mglDataA &x, const mglDataA &y, const mglDataA &ey, const (char*) pen="", const (char*) opt="")
	{	mgl_error_xy(gr, &x, &y, &ey, pen, opt);	}
	void Error(const mglDataA &x, const mglDataA &y, const mglDataA &ex, const mglDataA &ey, const (char*) pen="", const (char*) opt="")
	{	mgl_error_exy(gr, &x, &y, &ex, &ey, pen, opt);	}
	/// Draw marks with size r at points {x,y,z}
	void Mark(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &r, const (char*) pen, const (char*) opt="")
	{	mgl_mark_xyz(gr, &x, &y, &z, &r, pen, opt);	}
	void Mark(const mglDataA &x, const mglDataA &y, const mglDataA &r, const (char*) pen, const (char*) opt="")
	{	mgl_mark_xy(gr, &x, &y, &r, pen, opt);	}
	void Mark(const mglDataA &y, const mglDataA &r, const (char*) pen, const (char*) opt="")
	{	mgl_mark_y(gr, &y, &r, pen, opt);	}
	/// Draw textual marks with size r at points {x,y,z}
	void TextMark(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &r, const (char*) text, const (char*) fnt="", const (char*) opt="")
	{	mgl_textmark_xyzr(gr, &x, &y, &z, &r, text, fnt, opt);	}
	void TextMark(const mglDataA &x, const mglDataA &y, const mglDataA &r, const (char*) text, const (char*) fnt="", const (char*) opt="")
	{	mgl_textmark_xyr(gr, &x, &y, &r, text, fnt, opt);	}
	void TextMark(const mglDataA &y, const mglDataA &r, const (char*) text, const (char*) fnt="", const (char*) opt="")
	{	mgl_textmark_yr(gr, &y, &r, text, fnt, opt);	}
	void TextMark(const mglDataA &y, const (char*) text, const (char*) fnt="", const (char*) opt="")
	{	mgl_textmark(gr, &y, text, fnt, opt);	}
	void TextMark(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &r, const wchar_t *text, const (char*) fnt="", const (char*) opt="")
	{	mgl_textmarkw_xyzr(gr, &x, &y, &z, &r, text, fnt, opt);	}
	void TextMark(const mglDataA &x, const mglDataA &y, const mglDataA &r, const wchar_t *text, const (char*) fnt="", const (char*) opt="")
	{	mgl_textmarkw_xyr(gr, &x, &y, &r, text, fnt, opt);	}
	void TextMark(const mglDataA &y, const mglDataA &r, const wchar_t *text, const (char*) fnt="", const (char*) opt="")
	{	mgl_textmarkw_yr(gr, &y, &r, text, fnt, opt);	}
	void TextMark(const mglDataA &y, const wchar_t *text, const (char*) fnt="", const (char*) opt="")
	{	mgl_textmarkw(gr, &y, text, fnt, opt);	}

	/// Draw labels for points coordinate(s) at points {x,y,z}
	void Label(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) text, const (char*) fnt="", const (char*) opt="")
	{	mgl_label_xyz(gr, &x, &y, &z, text, fnt, opt);	}
	void Label(const mglDataA &x, const mglDataA &y, const (char*) text, const (char*) fnt="", const (char*) opt="")
	{	mgl_label_xy(gr, &x, &y, text, fnt, opt);	}
	void Label(const mglDataA &y, const (char*) text, const (char*) fnt="", const (char*) opt="")
	{	mgl_label_y(gr, &y, text, fnt, opt);	}
	void Label(const mglDataA &x, const mglDataA &y, const mglDataA &z, const wchar_t *text, const (char*) fnt="", const (char*) opt="")
	{	mgl_labelw_xyz(gr, &x, &y, &z, text, fnt, opt);	}
	void Label(const mglDataA &x, const mglDataA &y, const wchar_t *text, const (char*) fnt="", const (char*) opt="")
	{	mgl_labelw_xy(gr, &x, &y, text, fnt, opt);	}
	void Label(const mglDataA &y, const wchar_t *text, const (char*) fnt="", const (char*) opt="")
	{	mgl_labelw_y(gr, &y, text, fnt, opt);	}

	/// Draw table for values val along given direction with row labels text
	void Table(const mglDataA &val, const (char*) text, const (char*) fnt="#|", const (char*) opt="")
	{	mgl_table(gr, 0, 0, &val, text, fnt, opt);	}
	void Table(const mglDataA &val, const wchar_t *text, const (char*) fnt="#|", const (char*) opt="")
	{	mgl_tablew(gr, 0, 0, &val, text, fnt, opt);	}
	/// Draw table for values val along given direction with row labels text at given position
	void Table(double x, double y, const mglDataA &val, const (char*) text, const (char*) fnt="#|", const (char*) opt="")
	{	mgl_table(gr, x, y, &val, text, fnt, opt);	}
	void Table(double x, double y, const mglDataA &val, const wchar_t *text, const (char*) fnt="#|", const (char*) opt="")
	{	mgl_tablew(gr, x, y, &val, text, fnt, opt);	}

	/// Draw tube with radius r around curve {x,y,z}
	void Tube(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &r, const (char*) pen="", const (char*) opt="")
	{	mgl_tube_xyzr(gr, &x, &y, &z, &r, pen, opt);	}
	void Tube(const mglDataA &x, const mglDataA &y, const mglDataA &z, double r, const (char*) pen="", const (char*) opt="")
	{	mgl_tube_xyz(gr, &x, &y, &z, r, pen, opt);	}
	void Tube(const mglDataA &x, const mglDataA &y, const mglDataA &r, const (char*) pen="", const (char*) opt="")
	{	mgl_tube_xyr(gr, &x, &y, &r, pen, opt);	}
	void Tube(const mglDataA &x, const mglDataA &y, double r, const (char*) pen="", const (char*) opt="")
	{	mgl_tube_xy(gr, &x, &y, r, pen, opt);	}
	void Tube(const mglDataA &y, const mglDataA &r, const (char*) pen="", const (char*) opt="")
	{	mgl_tube_r(gr, &y, &r, pen, opt);	}
	void Tube(const mglDataA &y, double r, const (char*) pen="", const (char*) opt="")
	{	mgl_tube(gr, &y, r, pen, opt);	}
	/// Draw surface of curve {r,z} rotatation around axis
	void Torus(const mglDataA &r, const mglDataA &z, const (char*) pen="", const (char*) opt="")
	{	mgl_torus(gr, &r, &z, pen,opt);	}

	/// Draw mesh lines for 2d data specified parametrically
	void Mesh(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_mesh_xy(gr, &x, &y, &z, stl, opt);	}
	void Mesh(const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_mesh(gr, &z, stl, opt);	}
	/// Draw mesh lines for 2d data specified parametrically
	void Fall(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_fall_xy(gr, &x, &y, &z, stl, opt);	}
	void Fall(const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_fall(gr, &z, stl, opt);	}
	/// Draw belts for 2d data specified parametrically
	void Belt(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_belt_xy(gr, &x, &y, &z, stl, opt);	}
	void Belt(const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_belt(gr, &z, stl, opt);	}
	/// Draw surface for 2d data specified parametrically with color proportional to z
	void Surf(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_surf_xy(gr, &x, &y, &z, stl, opt);	}
	void Surf(const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_surf(gr, &z, stl, opt);	}
	/// Draw grid lines for density plot of 2d data specified parametrically
	void Grid(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_grid_xy(gr, &x, &y, &z, stl, opt);	}
	void Grid(const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_grid(gr, &z, stl, opt);	}
	/// Draw vertical tiles for 2d data specified parametrically
	void Tile(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_tile_xy(gr, &x, &y, &z, stl, opt);	}
	void Tile(const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_tile(gr, &z, stl, opt);	}
	/// Draw density plot for 2d data specified parametrically
	void Dens(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_dens_xy(gr, &x, &y, &z, stl, opt);	}
	void Dens(const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_dens(gr, &z, stl, opt);	}
	/// Draw vertical boxes for 2d data specified parametrically
	void Boxs(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_boxs_xy(gr, &x, &y, &z, stl, opt);	}
	void Boxs(const mglDataA &z, const (char*) stl="", const (char*) opt="")
	{	mgl_boxs(gr, &z, stl, opt);	}

	/// Draw contour lines for 2d data specified parametrically
	void Cont(const mglDataA &v, const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_cont_xy_val(gr, &v, &x, &y, &z, sch, opt);	}
	void Cont(const mglDataA &v, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_cont_val(gr, &v, &z, sch, opt);	}
	void Cont(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_cont_xy(gr, &x, &y, &z, sch, opt);	}
	void Cont(const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_cont(gr, &z, sch, opt);	}
	/// Draw solid contours for 2d data specified parametrically
	void ContF(const mglDataA &v, const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_contf_xy_val(gr, &v, &x, &y, &z, sch, opt);	}
	void ContF(const mglDataA &v, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_contf_val(gr, &v, &z, sch, opt);	}
	void ContF(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_contf_xy(gr, &x, &y, &z, sch, opt);	}
	void ContF(const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_contf(gr, &z, sch, opt);	}
	/// Draw solid contours for 2d data specified parametrically with manual colors
	void ContD(const mglDataA &v, const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_contd_xy_val(gr, &v, &x, &y, &z, sch, opt);	}
	void ContD(const mglDataA &v, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_contd_val(gr, &v, &z, sch, opt);	}
	void ContD(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_contd_xy(gr, &x, &y, &z, sch, opt);	}
	void ContD(const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_contd(gr, &z, sch, opt);	}
	/// Draw contour tubes for 2d data specified parametrically
	void ContV(const mglDataA &v, const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_contv_xy_val(gr, &v, &x, &y, &z, sch, opt);	}
	void ContV(const mglDataA &v, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_contv_val(gr, &v, &z, sch, opt);	}
	void ContV(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_contv_xy(gr, &x, &y, &z, sch, opt);	}
	void ContV(const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_contv(gr, &z, sch, opt);	}

	/// Draw axial-symmetric isosurfaces for 2d data specified parametrically
	void Axial(const mglDataA &v, const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_axial_xy_val(gr, &v, &x, &y, &z, sch,opt);	}
	void Axial(const mglDataA &v, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_axial_val(gr, &v, &z, sch, opt);	}
	void Axial(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_axial_xy(gr, &x, &y, &z, sch, opt);	}
	void Axial(const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_axial(gr, &z, sch, opt);	}

	/// Draw grid lines for density plot at slice for 3d data specified parametrically
	void Grid3(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) stl="", double sVal=-1, const (char*) opt="")
	{	mgl_grid3_xyz(gr, &x, &y, &z, &a, stl, sVal, opt);	}
	void Grid3(const mglDataA &a, const (char*) stl="", double sVal=-1, const (char*) opt="")
	{	mgl_grid3(gr, &a, stl, sVal, opt);	}
	/// Draw density plot at slice for 3d data specified parametrically
	void Dens3(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) stl="", double sVal=-1, const (char*) opt="")
	{	mgl_dens3_xyz(gr, &x, &y, &z, &a, stl, sVal, opt);	}
	void Dens3(const mglDataA &a, const (char*) stl="", double sVal=-1, const (char*) opt="")
	{	mgl_dens3(gr, &a, stl, sVal, opt);	}

	/// Draw isosurface(s) for 3d data specified parametrically
	void Surf3(double Val, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) stl="", const (char*) opt="")
	{	mgl_surf3_xyz_val(gr, Val, &x, &y, &z, &a, stl, opt);	}
	void Surf3(double Val, const mglDataA &a, const (char*) stl="", const (char*) opt="")
	{	mgl_surf3_val(gr, Val, &a, stl, opt);	}
	void Surf3(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) stl="", const (char*) opt="")
	{	mgl_surf3_xyz(gr, &x, &y, &z, &a, stl, opt);	}
	void Surf3(const mglDataA &a, const (char*) stl="", const (char*) opt="")
	{	mgl_surf3(gr, &a, stl, opt);	}

	/// Draw a semi-transparent cloud for 3d data
	void Cloud(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) stl="", const (char*) opt="")
	{	mgl_cloud_xyz(gr, &x, &y, &z, &a, stl, opt);	}
	void Cloud(const mglDataA &a, const (char*) stl="", const (char*) opt="")
	{	mgl_cloud(gr, &a, stl, opt);	}

	/// Draw contour lines at slice for 3d data specified parametrically
	void Cont3(const mglDataA &v, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) sch="", double sVal=-1, const (char*) opt="")
	{	mgl_cont3_xyz_val(gr, &v, &x, &y, &z, &a, sch, sVal, opt);	}
	void Cont3(const mglDataA &v, const mglDataA &a, const (char*) sch="", double sVal=-1, const (char*) opt="")
	{	mgl_cont3_val(gr, &v, &a, sch, sVal, opt);	}
	void Cont3(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) sch="", double sVal=-1, const (char*) opt="")
	{	mgl_cont3_xyz(gr, &x, &y, &z, &a, sch, sVal, opt);	}
	void Cont3(const mglDataA &a, const (char*) sch="", double sVal=-1, const (char*) opt="")
	{	mgl_cont3(gr, &a, sch, sVal, opt);	}

	/// Draw solid contours at slice for 3d data specified parametrically
	void ContF3(const mglDataA &v, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) sch="", double sVal=-1, const (char*) opt="")
	{	mgl_contf3_xyz_val(gr, &v, &x, &y, &z, &a, sch, sVal, opt);	}
	void ContF3(const mglDataA &v, const mglDataA &a, const (char*) sch="", double sVal=-1, const (char*) opt="")
	{	mgl_contf3_val(gr, &v, &a, sch, sVal, opt);	}
	void ContF3(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) sch="", double sVal=-1, const (char*) opt="")
	{	mgl_contf3_xyz(gr, &x, &y, &z, &a, sch, sVal, opt);	}
	void ContF3(const mglDataA &a, const (char*) sch="", double sVal=-1, const (char*) opt="")
	{	mgl_contf3(gr, &a, sch, sVal, opt);	}

	/// Draw several isosurfaces for 3d beam in curvilinear coordinates
	void Beam(const mglDataA &tr, const mglDataA &g1, const mglDataA &g2, const mglDataA &a, double r, const (char*) stl=0, int flag=0, int num=3)
	{	mgl_beam(gr, &tr,&g1,&g2,&a,r,stl,flag,num);	}
	void Beam(double val, const mglDataA &tr, const mglDataA &g1, const mglDataA &g2, const mglDataA &a, double r, const (char*) stl=NULL, int flag=0)
	{	mgl_beam_val(gr,val,&tr,&g1,&g2,&a,r,stl,flag);	}

	/// Draw vertical tiles with variable size r for 2d data specified parametrically
	void TileS(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &r, const (char*) stl="", const (char*) opt="")
	{	mgl_tiles_xy(gr, &x, &y, &z, &r, stl, opt);	}
	void TileS(const mglDataA &z, const mglDataA &r, const (char*) stl="", const (char*) opt="")
	{	mgl_tiles(gr, &z, &r, stl, opt);	}
	/// Draw surface for 2d data specified parametrically with color proportional to c
	void SurfC(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &c, const (char*) sch="", const (char*) opt="")
	{	mgl_surfc_xy(gr, &x, &y, &z, &c, sch,opt);	}
	void SurfC(const mglDataA &z, const mglDataA &c, const (char*) sch="", const (char*) opt="")
	{	mgl_surfc(gr, &z, &c, sch,opt);	}
	/// Draw surface for 2d data specified parametrically with alpha proportional to c
	void SurfA(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &c, const (char*) sch="", const (char*) opt="")
	{	mgl_surfa_xy(gr, &x, &y, &z, &c, sch,opt);	}
	void SurfA(const mglDataA &z, const mglDataA &c, const (char*) sch="", const (char*) opt="")
	{	mgl_surfa(gr, &z, &c, sch,opt);	}

	/// Color map of matrix a to matrix b, both matrix can parametrically depend on coordinates
	void Map(const mglDataA &x, const mglDataA &y, const mglDataA &a, const mglDataA &b, const (char*) sch="", const (char*) opt="")
	{	mgl_map_xy(gr, &x, &y, &a, &b, sch, opt);	}
	void Map(const mglDataA &a, const mglDataA &b, const (char*) sch="", const (char*) opt="")
	{	mgl_map(gr, &a, &b, sch, opt);	}
	/// Draw density plot for spectra-gramm specified parametrically
	void STFA(const mglDataA &x, const mglDataA &y, const mglDataA &re, const mglDataA &im, int dn, const (char*) sch="", const (char*) opt="")
	{	mgl_stfa_xy(gr, &x, &y, &re, &im, dn, sch, opt);	}
	void STFA(const mglDataA &re, const mglDataA &im, int dn, const (char*) sch="", const (char*) opt="")
	{	mgl_stfa(gr, &re, &im, dn, sch, opt);	}

	/// Draw isosurface(s) for 3d data specified parametrically with alpha proportional to b
	void Surf3A(double Val, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const mglDataA &b, const (char*) stl="", const (char*) opt="")
	{	mgl_surf3a_xyz_val(gr, Val, &x, &y, &z, &a, &b, stl, opt);	}
	void Surf3A(double Val, const mglDataA &a, const mglDataA &b, const (char*) stl="", const (char*) opt="")
	{	mgl_surf3a_val(gr, Val, &a, &b, stl, opt);	}
	void Surf3A(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const mglDataA &b, const (char*) stl="", const (char*) opt="")
	{	mgl_surf3a_xyz(gr, &x, &y, &z, &a, &b, stl, opt);	}
	void Surf3A(const mglDataA &a, const mglDataA &b, const (char*) stl="", const (char*) opt="")
	{	mgl_surf3a(gr, &a, &b, stl, opt);	}

	/// Draw isosurface(s) for 3d data specified parametrically with color proportional to b
	void Surf3C(double Val, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const mglDataA &b, const (char*) stl="", const (char*) opt="")
	{	mgl_surf3c_xyz_val(gr, Val, &x, &y, &z, &a, &b, stl,opt);	}
	void Surf3C(double Val, const mglDataA &a, const mglDataA &b, const (char*) stl="", const (char*) opt="")
	{	mgl_surf3c_val(gr, Val, &a, &b, stl, opt);	}
	void Surf3C(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const mglDataA &b, const (char*) stl="", const (char*) opt="")
	{	mgl_surf3c_xyz(gr, &x, &y, &z, &a, &b, stl, opt);	}
	void Surf3C(const mglDataA &a, const mglDataA &b, const (char*) stl="", const (char*) opt="")
	{	mgl_surf3c(gr, &a, &b, stl, opt);	}

	/// Plot dew drops for vector field {ax,ay} parametrically depended on coordinate {x,y}
	void Dew(const mglDataA &x, const mglDataA &y, const mglDataA &ax, const mglDataA &ay, const (char*) sch="", const (char*) opt="")
	{	mgl_dew_xy(gr, &x, &y, &ax, &ay, sch, opt);	}
	void Dew(const mglDataA &ax, const mglDataA &ay, const (char*) sch="", const (char*) opt="")
	{	mgl_dew_2d(gr, &ax, &ay, sch, opt);	}
	/// Plot vectors at position {x,y,z} along {ax,ay,az} with length/color proportional to |a|
	void Traj(const mglDataA &x, const mglDataA &y, const mglDataA &ax, const mglDataA &ay, const (char*) sch="", const (char*) opt="")
	{	mgl_traj_xy(gr, &x, &y, &ax, &ay, sch, opt);	}
	void Traj(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &ax, const mglDataA &ay, const mglDataA &az, const (char*) sch="", const (char*) opt="")
	...
	void Vect(const mglDataA &ax, const mglDataA &ay, const (char*) sch="", const (char*) opt="")
	{	mgl_vect_2d(gr, &ax, &ay, sch, opt);	}
	void Vect(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &ax, const mglDataA &ay, const mglDataA &az, const (char*) sch="", const (char*) opt="")
	{	mgl_vect_xyz(gr, &x, &y, &z, &ax, &ay, &az, sch, opt);	}
	void Vect(const mglDataA &ax, const mglDataA &ay, const mglDataA &az, const (char*) sch="", const (char*) opt="")
	{	mgl_vect_3d(gr, &ax, &ay, &az, sch, opt);	}

	/// Draw vector plot at slice for 3d data specified parametrically
	void Vect3(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &ax, const mglDataA &ay, const mglDataA &az, const (char*) stl="", double sVal=-1, const (char*) opt="")
	{	mgl_vect3_xyz(gr, &x, &y, &z, &ax,&ay,&az, stl, sVal, opt);	}
	void Vect3(const mglDataA &ax, const mglDataA &ay, const mglDataA &az, const (char*) stl="", double sVal=-1, const (char*) opt="")
	...
	void Flow(const mglDataA &x, const mglDataA &y, const mglDataA &ax, const mglDataA &ay, const (char*) sch="", const (char*) opt="")
	{	mgl_flow_xy(gr, &x, &y, &ax, &ay, sch, opt);	}
	void Flow(const mglDataA &ax, const mglDataA &ay, const (char*) sch="", const (char*) opt="")
	{	mgl_flow_2d(gr, &ax, &ay, sch, opt);	}
	void Flow(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &ax, const mglDataA &ay, const mglDataA &az, const (char*) sch="", const (char*) opt="")
	{	mgl_flow_xyz(gr, &x, &y, &z, &ax, &ay, &az, sch, opt);	}
	void Flow(const mglDataA &ax, const mglDataA &ay, const mglDataA &az, const (char*) sch="", const (char*) opt="")
	...
	void FlowP(mglPoint p, const mglDataA &x, const mglDataA &y, const mglDataA &ax, const mglDataA &ay, const (char*) sch="", const (char*) opt="")
	{	mgl_flowp_xy(gr, p.x, p.y, p.z, &x, &y, &ax, &ay, sch, opt);	}
	void FlowP(mglPoint p, const mglDataA &ax, const mglDataA &ay, const (char*) sch="", const (char*) opt="")
	{	mgl_flowp_2d(gr, p.x, p.y, p.z, &ax, &ay, sch, opt);	}
	void FlowP(mglPoint p, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &ax, const mglDataA &ay, const mglDataA &az, const (char*) sch="", const (char*) opt="")
	{	mgl_flowp_xyz(gr, p.x, p.y, p.z, &x, &y, &z, &ax, &ay, &az, sch, opt);	}
	void FlowP(mglPoint p, const mglDataA &ax, const mglDataA &ay, const mglDataA &az, const (char*) sch="", const (char*) opt="")
	...
	void Grad(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &phi, const (char*) sch="", const (char*) opt="")
	{	mgl_grad_xyz(gr,&x,&y,&z,&phi,sch,opt);	}
	void Grad(const mglDataA &x, const mglDataA &y, const mglDataA &phi, const (char*) sch="", const (char*) opt="")
	{	mgl_grad_xy(gr,&x,&y,&phi,sch,opt);	}
	void Grad(const mglDataA &phi, const (char*) sch="", const (char*) opt="")
	...
	void Pipe(const mglDataA &x, const mglDataA &y, const mglDataA &ax, const mglDataA &ay, const (char*) sch="", double r0=0.05, const (char*) opt="")
	{	mgl_pipe_xy(gr, &x, &y, &ax, &ay, sch, r0, opt);	}
	void Pipe(const mglDataA &ax, const mglDataA &ay, const (char*) sch="", double r0=0.05, const (char*) opt="")
	{	mgl_pipe_2d(gr, &ax, &ay, sch, r0, opt);	}
	void Pipe(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &ax, const mglDataA &ay, const mglDataA &az, const (char*) sch="", double r0=0.05, const (char*) opt="")
	{	mgl_pipe_xyz(gr, &x, &y, &z, &ax, &ay, &az, sch, r0, opt);	}
	void Pipe(const mglDataA &ax, const mglDataA &ay, const mglDataA &az, const (char*) sch="", double r0=0.05, const (char*) opt="")
	{	mgl_pipe_3d(gr, &ax, &ay, &az, sch, r0, opt);	}

	/// Draw density plot for data at x = sVal
	void DensX(const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_dens_x(gr, &a, stl, sVal, opt);	}
	/// Draw density plot for data at y = sVal
	void DensY(const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_dens_y(gr, &a, stl, sVal, opt);	}
	/// Draw density plot for data at z = sVal
	void DensZ(const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_dens_z(gr, &a, stl, sVal, opt);	}
	/// Draw contour lines for data at x = sVal
	void ContX(const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_cont_x(gr, &a, stl, sVal, opt);	}
	void ContX(const mglDataA &v, const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_cont_x_val(gr, &v, &a, stl, sVal, opt);	}
	/// Draw contour lines for data at y = sVal
	void ContY(const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_cont_y(gr, &a, stl, sVal, opt);	}
	void ContY(const mglDataA &v, const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_cont_y_val(gr, &v, &a, stl, sVal, opt);	}
	/// Draw contour lines for data at z = sVal
	void ContZ(const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_cont_z(gr, &a, stl, sVal, opt);	}
	void ContZ(const mglDataA &v, const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_cont_z_val(gr, &v, &a, stl, sVal, opt);	}
	/// Draw solid contours for data at x = sVal
	void ContFX(const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_contf_x(gr, &a, stl, sVal, opt);	}
	void ContFX(const mglDataA &v, const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_contf_x_val(gr, &v, &a, stl, sVal, opt);	}
	/// Draw solid contours for data at y = sVal
	void ContFY(const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_contf_y(gr, &a, stl, sVal, opt);	}
	void ContFY(const mglDataA &v, const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_contf_y_val(gr, &v, &a, stl, sVal, opt);	}
	/// Draw solid contours for data at z = sVal
	void ContFZ(const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_contf_z(gr, &a, stl, sVal, opt);	}
	void ContFZ(const mglDataA &v, const mglDataA &a, const (char*) stl="", double sVal=mglNaN, const (char*) opt="")
	{	mgl_contf_z_val(gr, &v, &a, stl, sVal, opt);	}

	/// Draw curve for formula with x in x-axis range
	void FPlot(const (char*) fy, const (char*) stl="", const (char*) opt="")
	{	mgl_fplot(gr, fy, stl, opt);	}
	/// Draw curve for formulas parametrically depended on t in range [0,1]
	void FPlot(const (char*) fx, const (char*) fy, const (char*) fz, const (char*) stl, const (char*) opt="")
	{	mgl_fplot_xyz(gr, fx, fy, fz, stl, opt);	}
	/// Draw surface by formula with x,y in axis range
	void FSurf(const (char*) fz, const (char*) stl="", const (char*) opt="")
	{	mgl_fsurf(gr, fz, stl, opt);	}
	/// Draw surface by formulas parametrically depended on u,v in range [0,1]
	void FSurf(const (char*) fx, const (char*) fy, const (char*) fz, const (char*) stl, const (char*) opt="")
	{	mgl_fsurf_xyz(gr, fx, fy, fz, stl, opt);	}

	/// Draw triangle mesh for points in arrays {x,y,z} with specified color c.
	void TriPlot(const mglDataA &nums, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &c, const (char*) sch="", const (char*) opt="")
	{	mgl_triplot_xyzc(gr, &nums, &x, &y, &z, &c, sch, opt);	}
	void TriPlot(const mglDataA &nums, const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_triplot_xyz(gr, &nums, &x, &y, &z, sch, opt);	}
	void TriPlot(const mglDataA &nums, const mglDataA &x, const mglDataA &y, const (char*) sch="", const (char*) opt="")
	{	mgl_triplot_xy(gr, &nums, &x, &y, sch, opt);	}
	/// Draw quad mesh for points in arrays {x,y,z} with specified color c.
	void QuadPlot(const mglDataA &nums, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &c, const (char*) sch="", const (char*) opt="")
	{	mgl_quadplot_xyzc(gr, &nums, &x, &y, &z, &c, sch, opt);	}
	void QuadPlot(const mglDataA &nums, const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_quadplot_xyz(gr, &nums, &x, &y, &z, sch, opt);	}
	void QuadPlot(const mglDataA &nums, const mglDataA &x, const mglDataA &y, const (char*) sch="", const (char*) opt="")
	...
	void TriCont(const mglDataA &nums, const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_tricont_xyc(gr, &nums, &x, &y, &z, sch, opt);	}
	void TriContV(const mglDataA &v, const mglDataA &nums, const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_tricont_xycv(gr, &v, &nums, &x, &y, &z, sch, opt);	}
	void TriCont(const mglDataA &nums, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) sch="", const (char*) opt="")
	{	mgl_tricont_xyzc(gr, &nums, &x, &y, &z, &a, sch, opt);	}
	void TriContV(const mglDataA &v, const mglDataA &nums, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) sch="", const (char*) opt="")
	{	mgl_tricont_xyzcv(gr, &v, &nums, &x, &y, &z, &a, sch, opt);	}
	void TriCont(const mglDataA &v, const mglDataA &nums, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) sch="", const (char*) opt="")
	{	mgl_tricont_xyzcv(gr, &v, &nums, &x, &y, &z, &a, sch, opt);	}

	/// Draw contour tubes for triangle mesh for points in arrays {x,y,z} with specified color c.
	void TriContVt(const mglDataA &nums, const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_tricontv_xyc(gr, &nums, &x, &y, &z, sch, opt);	}
	void TriContVt(const mglDataA &nums, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) sch="", const (char*) opt="")
	{	mgl_tricontv_xyzc(gr, &nums, &x, &y, &z, &a, sch, opt);	}
	void TriContVt(const mglDataA &v, const mglDataA &nums, const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) sch="", const (char*) opt="")
	{	mgl_tricontv_xyzcv(gr, &v, &nums, &x, &y, &z, &a, sch, opt);	}

	/// Draw dots in points {x,y,z}.
	void Dots(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_dots(gr, &x, &y, &z, sch, opt);	}
	/// Draw semitransparent dots in points {x,y,z} with specified alpha a.
	void Dots(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) sch="", const (char*) opt="")
	{	mgl_dots_a(gr, &x, &y, &z, &a, sch, opt);	}
	/// Draw semitransparent dots in points {x,y,z} with specified color c and alpha a.
	void Dots(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &c, const mglDataA &a, const (char*) sch="", const (char*) opt="")
	{	mgl_dots_ca(gr, &x, &y, &z, &c, &a, sch, opt);	}
	/// Draw surface reconstructed for points in arrays {x,y,z}.
	void Crust(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) sch="", const (char*) opt="")
	{	mgl_crust(gr, &x, &y, &z, sch, opt);	}

	/// Fit data along x-direction for each data row. Return array with values for found formula.
	mglData Fit(const mglDataA &y, const (char*) eq, const (char*) vars, const (char*) opt="")
	{	return mglData(true,mgl_fit_1(gr, &y, eq,vars,0, opt));	}
	mglData Fit(const mglDataA &y, const (char*) eq, const (char*) vars, mglData &ini, const (char*) opt="")
	...
	mglData Fit2(const mglDataA &z, const (char*) eq, const (char*) vars, const (char*) opt="")
	{	return mglData(true,mgl_fit_2(gr, &z, eq, vars,0, opt));	}
	mglData Fit2(const mglDataA &z, const (char*) eq, const (char*) vars, mglData &ini, const (char*) opt="")
	...
	mglData Fit3(const mglDataA &a, const (char*) eq, const (char*) vars, const (char*) opt="")
	{	return mglData(true,mgl_fit_3(gr, &a, eq, vars,0, opt));	}
	mglData Fit3(const mglDataA &a, const (char*) eq, const (char*) vars, mglData &ini, const (char*) opt="")
	{	return mglData(true,mgl_fit_3(gr, &a, eq, vars, &ini, opt));	}
	/// Fit data along x-direction for each data row. Return array with values for found formula.
	mglData Fit(const mglDataA &x, const mglDataA &y, const (char*) eq, const (char*) vars, const (char*) opt="")
	{	return mglData(true,mgl_fit_xy(gr, &x, &y, eq, vars,0, opt));	}
	mglData Fit(const mglDataA &x, const mglDataA &y, const (char*) eq, const (char*) vars, mglData &ini, const (char*) opt="")
	{	return mglData(true,mgl_fit_xy(gr, &x, &y, eq, vars, &ini, opt));	}
	/// Fit data along x-, y-directions for each data slice. Return array with values for found formula.
	mglData Fit(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) eq, const (char*) vars, const (char*) opt="")
	{	return mglData(true,mgl_fit_xyz(gr, &x, &y, &z, eq, vars,0, opt));	}
	mglData Fit(const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) eq, const (char*) vars, mglData &ini, const (char*) opt="")
	{	return mglData(true,mgl_fit_xyz(gr, &x, &y, &z, eq, vars, &ini, opt));	}
	/// Fit data along along all directions. Return array with values for found formula.
	mglData Fit(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) eq, const (char*) vars, const (char*) opt="")
	{	return mglData(true,mgl_fit_xyza(gr, &x, &y, &z, &a, eq, vars,0, opt));	}
	mglData Fit(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) eq, const (char*) vars, mglData &ini, const (char*) opt="")
	...
	mglData FitS(const mglDataA &y, const mglDataA &s, const (char*) eq, const (char*) vars, const (char*) opt="")
	{	return mglData(true,mgl_fit_ys(gr, &y, &s, eq, vars,0, opt));	}
	mglData FitS(const mglDataA &y, const mglDataA &s, const (char*) eq, const (char*) vars, mglData &ini, const (char*) opt="")
	{	return mglData(true,mgl_fit_ys(gr, &y, &s, eq, vars, &ini, opt));	}
	mglData FitS(const mglDataA &x, const mglDataA &y, const mglDataA &s, const (char*) eq, const (char*) vars, const (char*) opt="")
	{	return mglData(true,mgl_fit_xys(gr, &x, &y, &s, eq, vars,0, opt));	}
	mglData FitS(const mglDataA &x, const mglDataA &y, const mglDataA &s, const (char*) eq, const (char*) vars, mglData &ini, const (char*) opt="")
	...
	mglData FitS(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &s, const (char*) eq, const (char*) vars, const (char*) opt="")
	{	return mglData(true,mgl_fit_xyzs(gr, &x, &y, &z, &s, eq, vars,0, opt));	}
	mglData FitS(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &s, const (char*) eq, const (char*) vars, mglData &ini, const (char*) opt="")
	...
	mglData FitS(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const mglDataA &s, const (char*) eq, const (char*) vars, const (char*) opt="")
	{	return mglData(true,mgl_fit_xyzas(gr, &x, &y, &z, &a, &s, eq, vars,0, opt));	}
	mglData FitS(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const mglDataA &s, const (char*) eq, const (char*) vars, mglData &ini, const (char*) opt="")
	{	return mglData(true,mgl_fit_xyzas(gr, &x, &y, &z, &a, &s, eq, vars, &ini, opt));	}
	/// Print fitted last formula (with coefficients)
	void PutsFit(mglPoint p, const (char*) prefix=0, const (char*) font="", double size=-1)
	...
	mglData PDE(const (char*) ham, const mglDataA &ini_re, const mglDataA &ini_im, double dz=0.1, double k0=100, const (char*) opt="")
	{	return mglData(true,mgl_pde_solve(gr,ham,&ini_re,&ini_im,dz,k0, opt));	}
	/// Fill data by formula with x,y,z in range [Min, Max]
	void Fill(mglData &u, const (char*) eq, const (char*) opt="")
	{	mgl_data_fill_eq(gr, &u, eq, 0, 0, opt);	}
	void Fill(mglData &u, const (char*) eq, const mglDataA &v, const (char*) opt="")
	{	mgl_data_fill_eq(gr, &u, eq, &v, 0, opt);	}
	void Fill(mglData &u, const (char*) eq, const mglDataA &v, const mglDataA &w, const (char*) opt="")
	{	mgl_data_fill_eq(gr, &u, eq, &v, &w, opt);	}
	/// Fill data by formula with x,y,z in range [Min, Max]
	void Fill(mglDataC &u, const (char*) eq, const (char*) opt="")
	{	mgl_datac_fill_eq(gr, &u, eq, 0, 0, opt);	}
	void Fill(mglDataC &u, const (char*) eq, const mglDataA &v, const (char*) opt="")
	{	mgl_datac_fill_eq(gr, &u, eq, &v, 0, opt);	}
	void Fill(mglDataC &u, const (char*) eq, const mglDataA &v, const mglDataA &w, const (char*) opt="")
	...
	void Refill(mglData &dat, const mglDataA &xdat, const mglDataA &vdat, long sl=-1, const (char*) opt="")
	{	mgl_data_refill_gr(gr,&dat,&xdat,0,0,&vdat,sl,opt);	}
	void Refill(mglData &dat, const mglDataA &xdat, const mglDataA &ydat, const mglDataA &vdat, long sl=-1, const (char*) opt="")
	{	mgl_data_refill_gr(gr,&dat,&xdat,&ydat,0,&vdat,sl,opt);	}
	void Refill(mglData &dat, const mglDataA &xdat, const mglDataA &ydat, const mglDataA &zdat, const mglDataA &vdat, const (char*) opt="")
	...
	void DataGrid(mglData &d, const mglDataA &x, const mglDataA &y, const mglDataA &z, const (char*) opt="")
	{	mgl_data_grid(gr,&d,&x,&y,&z,opt);	}

	/// Make histogram (distribution) of data. This function do not plot data.
	mglData Hist(const mglDataA &x, const mglDataA &a, const (char*) opt="")
	{	return mglData(true, mgl_hist_x(gr, &x, &a, opt));	}
	mglData Hist(const mglDataA &x, const mglDataA &y, const mglDataA &a, const (char*) opt="")
	{	return mglData(true, mgl_hist_xy(gr, &x, &y, &a, opt));	}
	mglData Hist(const mglDataA &x, const mglDataA &y, const mglDataA &z, const mglDataA &a, const (char*) opt="")
	{	return mglData(true, mgl_hist_xyz(gr, &x, &y, &z, &a, opt));	}

	void Compression(bool){}		// NOTE: Add later -- IDTF
	/// Set the preference for vertex color on/off (for formats that support it, now only PRC does).
	void VertexColor(bool enable)	{	mgl_set_flag(gr,enable, MGL_PREFERVC);	}
	/// Render only front side of surfaces for dubugging purposes (for formats that support it, now only PRC does).
	void DoubleSided(bool enable)	{	mgl_set_flag(gr,!enable, MGL_ONESIDED);	}
...
#pragma omp critical
	...
	/// Restore once flag
	void RestoreOnce()	{	mgl_parser_restore_once(pr);	}
	/// Allow changing size of the picture
	void AllowSetSize(bool allow)	{	mgl_parser_allow_setsize(pr, allow);	}
	/// Allow reading/saving files
	void AllowFileIO(bool allow)		{	mgl_parser_allow_file_io(pr, allow);	}
	/// Allow loading commands from external libraries
	void AllowDllCall(bool allow)	{	mgl_parser_allow_dll_call(pr, allow);	}
	/// Set flag to stop script parsing
	void Stop()	{	mgl_parser_stop(pr);	}

	/// Return result of formula evaluation
	mglData Calc(const (char*) formula)
	...
	void DeleteVar(const (char*) name)		{	mgl_parser_del_var(pr, name);		}
	void DeleteVar(const wchar_t *name)	{	mgl_parser_del_varw(pr, name);		}
